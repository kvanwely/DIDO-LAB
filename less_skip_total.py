...
#this script will take the output from the skipping and get general base composition for the exons skipped significantly less
...
def open_junctions (mats_file,test_junctions):
     
    del test_junctions[:]
        
    infile = open(mats_file,"r")
    for line in infile:
...
def get_fasta (cutoff,mats_file,fasta_prefix,transcripts,out_file):
    ...
    open_junctions(mats_file,test_junctions)
...
def main():
    ...
    parser.add_option("-m","--mats-file",action="store",type="string",dest="mats_file",help="mats results file")
    ...
    if not (options.cutoff and options.mats_file and options.fasta_prefix and options.transcripts and options.out_file):
        parser.print_help()
        sys.exit(0)
    else:
        get_fasta (options.cutoff,options.mats_file,options.fasta_prefix,options.transcripts,options.out_file)
