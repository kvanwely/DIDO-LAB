...
#this script is used to select valid junctions from a superfile generated by junc_superfile.py
...
def get_fasta (num_hits,juncs_file,out_file):
    ...
    for junc1 in all_junctions:
        ...
        overhangs1_total = int(overhang1_left) + int(overhang1_right)
                    
        if int(overhang1_left) > 10 and  int(overhang1_right) > 10 and junc1_depth_val > 3:
            print >>outfile, junc1
...
def main():
    ...
    if not (options.num_hits and options.juncs_file and options.out_file):
        parser.print_help()
        sys.exit(0)
    else:
        get_fasta (options.num_hits,options.juncs_file,options.out_file)
